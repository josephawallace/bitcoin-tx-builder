...
let children: Array<Child> = [], index = 0, unusedCount = 0;
...
const getAddressTxRecords = async (child: Child) => {
        ...
        for (let index = 0; index < tx.vout.length; index++) { 
            if (tx.vout[index].scriptpubkey_address == child.address) {
                ...
                txRecords.push({
                    ...
                    hash: tx.txid,
                    index: index,
                    nonWitnessUtxo: txhex, // have to make conversion to buffer when using as tx input
...
const createTransaction = (address: string, outputValue: number, transactionFee: number) => {
    let utxoValueSum = 0, signers = [];
    const psbt = new bitcoin.Psbt({ network: TESTNET });
    txos.forEach((txo) => {
        if (txo.spent || utxoValueSum >= outputValue + transactionFee) { return; }
